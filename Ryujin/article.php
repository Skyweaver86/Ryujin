    ...
    if ($invalid) {                                              // If invalid
        ...
        try {                                                    // Try to insert data
            ...
            if ($destination) {                                  // If have valid image
                // Crop and save file
                // Resize and save using GD (built into XAMPP, no extra install needed)
                ...
                imagecopyresampled($tmp, $src, 0, 0, 0, 0, $new_w, $new_h, $orig_w, $orig_h);
                $dst = imagecreatetruecolor($target_w, $target_h);
                $offset_x = (int)(($new_w - $target_w) / 2);
                $offset_y = (int)(($new_h - $target_h) / 2);
                imagecopy($dst, $tmp, 0, 0, $offset_x, $offset_y, $target_w, $target_h);
...
?>
<?php $page_title = $id ? 'Edit Article' : 'Add Article'; $breadcrumbs = ['Articles' => 'articles.php', ($id ? 'Edit' : 'Add') => null]; ?>
<?php include '../includes/admin-header.php'; ?>
  <form action="article.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data">


      <div class="page-header" style="grid-column:1/-1"><h1><?= $id ? 'Edit Article' : 'Add Article' ?></h1></div>
